--[CEDO] Yatogami Tohka - Spacequakes Level 1
--Scripted by Raivost (Ravi)
local s,id=GetID()
function s.initial_effect(c)
    --(1) Conjure (From Hand) -> Destroy 1 Enemy Monster
...
function s.conjure_condition_filter(c,e,tp)
	...
	return c:IsSummonPlayer(tp) and c:IsPreviousLocation(LOCATION_HAND) and c:IsLevel(hero_rank) and s[tp]==1
end
...
function s.checkop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local hero_rank=e:GetHandler():GetRank()
    while tc do
    if tc:IsType(TYPE_MONSTER) and tc:IsPreviousLocation(LOCATION_HAND) and tc:IsLevel(hero_rank) then
